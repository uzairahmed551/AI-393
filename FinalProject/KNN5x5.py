...
X = X.to_numpy()
print(X.shape) #printing

sX = np.empty((0,576), int)


ss = 28000 
#for loop to test this output
for img in X[0:ss,:]:
...
print(sY.shape) #printing y axis shape
print(sX.shape)#printing X axis shape
...
print('Length ==> ',len(Y_test)) #PrintlN
print('K: ',np.sqrt(len(Y_test))) #PrintK

classifier = KNeighborsClassifier(n_neighbors=7,p=2,metric='euclidean')
classifier.fit(X_train,Y_train)
Y_pred = classifier.predict(X_test)
print(classification_report(Y_test,Y_pred))
print("Accuracy of KNN ==: ", end= ' ')
